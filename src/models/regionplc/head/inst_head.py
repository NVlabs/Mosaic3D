...
class InstHead(nn.Module):
    def __init__(
        ...
        label_shift,
        ignore_label=-100,
        base_inst_class_idx=None,
        novel_inst_class_idx=None,
    ):
        super().__init__()
        ...
        self.offset_linear = MLP(
            [self.mid_channel, self.mid_channel, 3], norm_fn=norm_fn, num_layers=2
        )
        ...
        self.tiny_unet = UBlock(
            [self.mid_channel, 2 * self.mid_channel], norm_fn, 2, block, indice_key_id=11
        )
    ...
    def forward_grouping(
        ...
        **kwargs
        ...
        if (
            not self.training
            and (not pseudo_label_generation)
            and self.correct_seg_pred_binary
            and binary_scores is not None
        ):
        ...
        for cic, cio in zip(class_id_curr, class_id_origin):  # only training
            ...
            cls_scores = scatter_mean(
                _semantic_scores[proposals_idx[:, 1].long().cuda()],
                index=proposals_idx[..., 0].long().cuda(),
                dim=0,
            )
            if (self.training or pseudo_label_generation) and hasattr(self, "inst_base_class_idx"):
                proposals_sem_scores.append(
                    cls_scores[
                        ..., self.label_shift : self.label_shift + len(self.inst_base_class_idx)
                    ]
                )
            else:
            ...
            if binary_scores is not None:
                _binary_scores = scatter_mean(
                    binary_scores[proposals_idx[:, 1].long().cuda()],
                    index=proposals_idx[..., 0].long().cuda(),
                    dim=0,
                )
        ...
        if pseudo_label_generation:
            for ii in range(0, 1):  # base: 1, novel: 0
                ...
                if super_voxel is not None:
                    ...
                    sv_clustering = AgglomerativeClustering(
                        distance_threshold=self.pseudo_offset_cfg.CLUSTER_THR, n_clusters=None
                    ).fit(sv_feats.detach().cpu().numpy())
                ...
                for jj in cluster_ids:
                    ...
                    proposals_idx, proposals_offset = ops.bfs_cluster(
                        class_numpoint_mean, idx.cpu(), start_len.cpu(), npoint_thr, 0
                    )  # class_id set to zero
                    proposals_idx[:, 1] = object_idxs[proposals_idx[:, 1].long()].int()
    ...
    def get_instances(
        ...
        if self.correct_seg_pred_binary and binary_scores is not None:
            ...
            sigmoid_binary_scores = torch.sigmoid(
                proposal_binary_scores / 1.0
            )  # novel: 0, base: 1
            sigmoid_binary_scores = sigmoid_binary_scores.repeat(1, cls_scores.shape[1])
            sigmoid_binary_scores[..., self.inst_novel_class_idx] = (
                1 - sigmoid_binary_scores[..., self.inst_novel_class_idx]
            )
    ...
    def panoptic_fusion(self, semantic_preds, instance_preds):
        ...
        ignore_inds = (
            (panoptic_cls >= self.inst_class_idx.min())
            & (panoptic_cls <= self.inst_class_idx.max())
            & (panoptic_ids == 0)
        )
    ...
    def forward(self, batch_dict):
        ...
        semantic_scores, batch_idxs, points, batch_size = (
            batch_dict["seg_scores"],
            batch_dict["batch_idxs"],
            batch_dict["points_xyz"],
            batch_dict["batch_size"],
        )
        binary_scores = (
            batch_dict["binary_ret_dict"]["binary_scores"]
            if "binary_ret_dict" in batch_dict
            else None
        )
        ...
        if not self.semantic_only:
            if batch_dict.get("gt_label_generation", False):
                ...
                self.forward_ret_dict["pt_offsets"][valid_idx] = (
                    batch_dict["pt_offset_label"][valid_idx] * 0.5
                )
                ...
                ) = self.forward_grouping(
                    batch_size,
                    semantic_scores,
                    pt_offsets,
                    batch_idxs,
                    points,
                    super_voxel=batch_dict.get("super_voxel", None),
                    output_feats=backbone3d_feats,
                ...
                if len(proposals_idx) > 0:
                    proposals_center = scatter_mean(
                        points.cpu()[proposals_idx[..., 1].long()],
                        index=proposals_idx[..., 0].long(),
                        dim=0,
                    )
                    points_center = proposals_center[proposals_idx[..., 0].long()]
                    points_center = scatter_mean(
                        points_center, index=proposals_idx[..., 1].long(), dim=0
                    )  # (N, C)
                    if points_center.shape[0] < points.shape[0]:
                        remain_points = points.shape[0] - points_center.shape[0]
                        points_center = torch.cat(
                            (points_center, torch.zeros((remain_points, 3))), 0
                        )
                    point_offset = points_center - points.cpu()
                    point_offset[(points_center == 0).sum(1) == 3] = 0.0
                else:
            ...
            if self.training and len(proposals_idx) == 0:
                ...
                while len(proposals_idx) == 0 and point_nth_multiplier > 0:
                    ...
                    (
                        ...
                        points,
                        binary_scores=binary_scores,
                        point_nth_multiplier=point_nth_multiplier,
                    )
            ...
            inst_feats, inst_map = self.clusters_voxelization(
                ...
                backbone3d_feats,
                points,
                rand_quantize=self.training,
                **self.inst_voxel_cfg
            )

            inst_batch_idxs, iou_scores, mask_scores = self.forward_instance(inst_feats, inst_map)
            if self.training:
            ...
            if not self.training:
                pred_instances, _ = self.get_instances(
                    ...
                    iou_scores,
                    mask_scores,
                    proposal_binary_scores=proposal_binary_scores,
                    binary_scores=binary_scores,
                )
                if "instance" in self.test_cfg.EVAL_TASKS:
                    gt_instances = self.get_gt_instances(
                        batch_dict["labels"].clone(), batch_dict["inst_label"].clone()
                    )
                ...
                if "panoptic" in self.test_cfg.EVAL_TASKS:
                    panoptic_preds = self.panoptic_fusion(
                        batch_dict["seg_preds"].cpu().numpy(), pred_instances
                    )
    ...
    def get_offset_loss(self, pt_offsets, pt_offset_labels, inst_labels, pos_inds=None):
        ...
        if pos_inds.sum() == 0:
            ...
            if self.offset_loss_type == "l1":
                ...
                offset_loss = loss_utils.CosineSimilarityLoss()(
                    pt_offsets, pt_offset_labels[pos_inds], pos_inds
                )
        return offset_loss
    ...
    def get_inst_loss(
        ...
        mask_label = ops.get_mask_label(
            proposals_idx[:, 1].contiguous(),
            proposals_offset,
            _inst_labels,
            inst_cls,
            inst_pointnum,
            ious_on_cluster,
            self.loss_cfg.POS_IOU_THR,
        )
    ...
    def get_loss(self):
        ...
        if not self.semantic_only and "proposals_offset" in self.forward_ret_dict:
            ...
            tb_dict.update(
                {"mask_loss": mask_loss.item(), "iou_score_loss": iou_score_loss.item()}
            )
        return loss, tb_dict
