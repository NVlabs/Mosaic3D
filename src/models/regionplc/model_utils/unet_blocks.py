...
class UBlock(nn.Module):
    def __init__(self, nPlanes, norm_fn, block_reps, block, indice_key_id=1):
        ...
        blocks = {
            f"block{i}": block(nPlanes[0], nPlanes[0], norm_fn, indice_key=f"subm{indice_key_id}")
...
class UBlockDecoder(nn.Module):
    ...
    def forward(self, input_list):
        ...
        if self.detach:
            identity = spconv.SparseConvTensor(
                output.features.detach(), output.indices, output.spatial_shape, output.batch_size
            )
        else:
            identity = spconv.SparseConvTensor(
                output.features, output.indices, output.spatial_shape, output.batch_size
