...
def load_text_embedding_from_path(text_emb_path, logger):
    ...
    #         common_utils.oss_data_client.get(text_emb_path), map_location=torch.device("cpu")
    #     ).detach()
    # else:
    if True:
    ...
    logger.info(f"=> loaded text embedding from path '{text_emb_path}'")
...
#         text_embedding /= text_embedding.norm(dim=-1, keepdim=True)
#     logger.info(f"=> loaded text embedding from '{model_cfg.NAME}'")
