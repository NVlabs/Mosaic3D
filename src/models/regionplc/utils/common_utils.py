...
def limit_period(val, offset=0.5, period=np.pi):
    ...
    answer = val - torch.floor(val / period + offset) * period
    return answer.numpy() if is_numpy else answer
...
def update_binary_acc_meter(
    intersection_meter, target_meter, binary_preds, labels, idx1, n_classes
):
    # idx1: binary_label: 0
    ...
    for i, idx in enumerate(ids):
        if "txt" in formats:
            ...
            save_path = os.path.join(
                result_dir, "txt", data_list[idx].split("/")[-1].split(".")[0] + ".txt"
            )
            ...
            if isinstance(preds, torch.FloatTensor):
                ...
                np.savetxt(
                    save_path, preds[offsets[i] : offsets[i + 1]].astype(np.int64), fmt="%d"
                )
        ...
        if "npy" in formats:
            save_path = os.path.join(
                result_dir, "npy", data_list[idx].split("/")[-1].split(".")[0] + ".npy"
            )
            if os.path.exists(save_path) and not replace:
                continue
