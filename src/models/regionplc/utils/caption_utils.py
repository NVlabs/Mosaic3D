...
def select_images(caption_cfg, image_name, image_corr):
    ...
    if caption_cfg.get("SAMPLE", 1) > 1:
        ...
        image_name = [
            (np.array(image_name[i])[random_start :: caption_cfg.SAMPLE]).tolist()
            for i in range(batch_size)
        ]
        image_corr = [
            (np.array(image_corr[i], dtype=object)[random_start :: caption_cfg.SAMPLE]).tolist()
            for i in range(batch_size)
        ]
    ...
    for i in range(batch_size):
        if image_name[i] is None or len(image_name[i]) == 0:  # lack 2d data
            ...
            selected_idx = np.random.choice(
                len(image_name[i]), min(num, len(image_name[i])), replace=False
            )
        elif caption_cfg.SELECT == "ratio":
            # sequence slicing
            ratio = caption_cfg.RATIO
            selected_idx = np.random.choice(
                len(image_name[i]), max(1, int(len(image_name[i]) * ratio)), replace=False
            )
        elif caption_cfg.SELECT == "hybrid":
            num = max(int(caption_cfg.NUM), int(len(image_name[i]) * caption_cfg.RATIO))
            selected_idx = np.random.choice(
                len(image_name[i]), min(max(1, num), len(image_name[i])), replace=False
            )
        else:
            raise NotImplementedError
...
def enlarge_boxes_size(origin_boxes, enlarge_ratio, max_box_size, image_shape):
    ...
    boxes_size = (origin_boxes[:, 2] - origin_boxes[:, 0]) * (
        origin_boxes[:, 3] - origin_boxes[:, 1]
    )
