...
class Muon(Optimizer):
    ...
    Arguments:
        ...
        adam_lr: Learning rate for Adam optimization (parameters with ndim < 2).
        adam_weight_decay: Weight decay for Adam optimization (parameters with ndim < 2).
        betas: Adam betas for parameters with ndim < 2.
        eps: Adam epsilon for parameters with ndim < 2.
    """
    ...
    def __init__(
        ...
        # Group Muon parameters by size for distributed optimization
        for size in {p.numel() for p in muon_params}:
        ...
        # Group Adam parameters (no special buffering needed)
        if adam_params:
            group = dict(
                params=adam_params,
                use_muon=False,
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            if group.get("use_muon", True):
                # Muon optimization for ndim >= 2 parameters
