...
class Mask3D(nn.Module):
    def __init__(
        ...
        if self.non_parametric_queries:
            ...
            if self.use_np_features:
                self.np_feature_projection = nn.Sequential(
                    nn.Linear(sizes[-1], hidden_dim), nn.ReLU(), nn.Linear(hidden_dim, hidden_dim)
                )
        ...
        if self.pos_enc_type == "legacy":
            ...
            self.pos_enc = PositionEmbeddingCoordsSine(
                pos_type="sine", d_pos=self.mask_dim, normalize=self.normalize_pos_enc
            )
        else:
        ...
        for i in range(len(coords)):
            ...
            for coords_batch in coords[i].decomposed_features:
                ...
                with autocast(enabled=False):
                    tmp = self.pos_enc(
                        coords_batch[None, ...].float(), input_range=[scene_min, scene_max]
                    )
        ...
        if self.non_parametric_queries:
            fps_idx = [
                furthest_point_sample(
                    x.decomposed_coordinates[i][None, ...].float(), self.num_queries
                )
                .squeeze(0)
                .long()
            ...
            query_pos = (
                torch.rand(batch_size, self.mask_dim, self.num_queries, device=x.device) - 0.5
            )
            ...
            if not self.random_normal:
                query_pos_feat = (
                    torch.rand(batch_size, 2 * self.mask_dim, self.num_queries, device=x.device)
                    - 0.5
                )
            else:
                query_pos_feat = torch.randn(
                    batch_size, 2 * self.mask_dim, self.num_queries, device=x.device
                )
            ...
            for i, hlevel in enumerate(self.hlevels):
                ...
                for k in range(len(decomposed_aux)):
                    ...
                    if pcd_size <= curr_sample_size:
                        ...
                        idx = torch.zeros(
                            curr_sample_size, dtype=torch.long, device=queries.device
                        )

                        midx = torch.ones(
                            curr_sample_size, dtype=torch.bool, device=queries.device
                        )
