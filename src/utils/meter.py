...
class GPUMemoryMeter:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        self.num_calls += 1
        self.average_time = self.elapsed_time / self.num_calls
...
def load_test_data(path: str = "data/drivaer_0.npy", device: str = "cuda:0"):
    vertices = np.load(path)
    points = torch.tensor(vertices, dtype=torch.float32).to(device)
    return points


if __name__ == "__main__":
    # Test the meter
    meter = GPUMemoryMeter()
    for _ in range(10):
        with meter:
