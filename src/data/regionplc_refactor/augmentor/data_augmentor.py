...
class DataAugmentor:
    ...
    def crop(self, data_dict=None, config=None):
        data_dict["points_xyz_voxel_scale"], valid_idxs = augmentor_utils.crop(
            data_dict["points_xyz_voxel_scale"],
            self.kwargs["full_scale"],
            self.kwargs["max_npoint"],
            config.step,
        )
    ...
    def scene_aug(self, data_dict=None, config=None):
        if self.check_func(config) and self.check_data(data_dict):
            (
                ...
                data_dict["pseudo_offset_target"],
                data_dict["rgb"],
            ) = augmentor_utils.scene_aug(
                config,
                data_dict["points_xyz"],
                data_dict.get("pseudo_offset_target", None),
                data_dict["rgb"],
            )
            if data_dict["points_xyz"].shape[0] == 0:
                data_dict["valid"] = False
    ...
    def elastic(self, data_dict=None, config=None):
        data_dict["points_xyz_voxel_scale"] = (
            data_dict["points_xyz"] * self.kwargs["voxel_scale"] / self.kwargs["voxel_down"]
        )
        if self.check_func(config) and self.check_data(data_dict):
    ...
    def random_world_rotation(data_dict=None, config=None):
        ...
        if data_dict.get("pseudo_offset_target", None) is not None:
            data_dict["pseudo_offset_target"][:, :2] = np.dot(
                data_dict["pseudo_offset_target"][:, :2], j
            )
        return data_dict
    ...
    def random_world_translation(data_dict=None, config=None):
        ...
        noise_translate = np.array(
            [
                np.random.normal(0, config[0], 1),
                np.random.normal(0, config[1], 1),
                np.random.normal(0, config[2], 1),
            ]
        ).T
